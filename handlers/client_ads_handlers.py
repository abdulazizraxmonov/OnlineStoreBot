...
@ads_router.message()
async def search_ads_by_text(message: Message):
    text = message.text
    if len(text) < 1:
        await message.answer("1 hariflik elon bolishi munkun emas")
        return
    matching_ads = db.search_ads_by_text(text)
    if matching_ads:
        results_text = "Qidiruv Natijasi:\n"
        for i, ad in enumerate(matching_ads, 1):
            results_text += f"{i}: {ad['ad_title']}\n"

        inline_keyboard = []
        row = []
        for j, ad in enumerate(matching_ads, 1):
            row.append(InlineKeyboardButton(text=str(j), callback_data=f"select_ad_{ad['id']}_details"))
            if j % 3 == 0 or j == len(matching_ads):
                inline_keyboard.append(row)
                row = []

        reply_markup = InlineKeyboardMarkup(inline_keyboard=inline_keyboard)
        await message.answer(results_text, reply_markup=reply_markup)
    else:
        await message.answer("Hech narsa topilmadi🙄.")

@ads_router.callback_query(lambda c: c.data.startswith('select_ad_'))
async def select_ad_callback(callback: CallbackQuery, state: FSMContext):
    selected_id_str = callback.data.split('_')[2]
    if not selected_id_str.isdigit():
        await callback.message.answer("notogri soz yozdingiz.")
        return
    selected_id = int(selected_id_str)
    matching_ad = db.search_ads_by_id(selected_id)
    if matching_ad:
        selected_ad = matching_ad[0]
        ad_info = f"<b>{selected_ad['ad_title']}</b>\n\n{selected_ad['ad_text']}\n\nPrice: ${selected_ad['ad_price']}"
        photo_id = selected_ad['ad_images']
        await callback.message.answer_photo(photo=photo_id, caption=ad_info, parse_mode=ParseMode.HTML)
    else:
        await callback.message.answer("tanlangan malumot bazada topilmadi.")
